...
def run_recognition(video_source=None, on_identify=None):
    ...
    # Get FPS for video speed control
    fps = cap.get(cv2.CAP_PROP_FPS)
    if not fps or fps <= 0: fps = 30 # Default to 30 if unavailable
    ...
    while True:
        ...
        if int(cap.get(cv2.CAP_PROP_POS_FRAMES)) % FRAME_SKIP == 0:
            current_faces = identify_face(frame, known_encodings, known_names, known_guardian_nos, known_guardian_names, known_school_names)
            ...
            for face in current_faces:
                ...
                if tracker[name]["count"] >= CONFIRMATION_THRESHOLD and name not in announced:
                    if on_identify: on_identify(name, tracker[name]["guardian_no"], tracker[name]["guardian_name"], tracker[name]["school_name"])
                    announced.add(name)
